...
async function seedInventory() {
  ...
  const newCount = inventoryData.filter(i => i.type === 'new').length;
  const refurbishedCount = inventoryData.filter(i => i.type === 'refurbished').length;

  console.log('\nInventory Summary:');
  console.log(`  Total Products: ${inventoryData.length}`);
  console.log(`  Total Units: ${totalUnits}`);
  console.log(`  New: ${newCount}`);
  console.log(`  Refurbished: ${refurbishedCount}`);
}
